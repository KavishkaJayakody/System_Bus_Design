...
module tb_top_bus_system;
    ...
    initial begin
        ...
        // ---------------------------------------------------------------
        // TEST 3: Randomized Accesses to External Bridge Register (0x3000)
        // ---------------------------------------------------------------
        $display("\n[TEST 3] External Bridge Register Reads (%0d Randomized Trials)...", NUM_RANDOM_TRIALS);
        for (i = 0; i < NUM_RANDOM_TRIALS; i = i + 1) begin : test3_loop
            reg [7:0] dummy_wdata;
            dummy_wdata = $random & 8'hFF; // Seed dummy write data to ensure bus stability

            execute_m0(1'b0, BRIDGE_ADDR, dummy_wdata);
            @(posedge m0_cmd_done);

            if (m0_cmd_rdata !== 8'hBE) begin
                $display("-> ERROR [Test 3 Iter %0d]: Expected default 0xBE, got 0x%02X", i, m0_cmd_rdata);
                error_count = error_count + 1;
            end
        end
